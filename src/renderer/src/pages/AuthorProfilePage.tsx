...
export function AuthorProfilePage(): React.ReactElement {
  ...
  useEffect(() => {
    if (!authorId || !supabase) { setLoading(false); return }
    let cancelled = false
    setLoading(true)
    void (async () => {
      const [profileRes, scriptsRes] = await Promise.all([
        supabase.from('profiles').select('id, display_name').eq('id', authorId).maybeSingle(),
        supabase.from('scripts').select('id, slug, title, description, tags, category, content_version, updated_at, status, filename, author_id')
          .eq('author_id', authorId).eq('status', 'published').order('updated_at', { ascending: false }),
      ])
      if (cancelled) return
      ...
      setScripts((scriptsRes.data as ScriptRow[]) ?? [])
      setLoading(false)
    })()
  ...
  if (!profile) return <div className="page"><p className="muted">Author not found.</p><Link to="/">Store</Link></div>
  ...
  return (
    <div className="page">
      ...
      <div className="row-list">
        {scripts.map((s) => (
          <Link key={s.id} to={`/script/${s.slug}`} className="script-row" style={{ textDecoration: 'none', color: 'inherit' }}>
            <div className="row-info">
              <div className="row-name">{s.title}</div>
              <div className="row-desc line-clamp">{s.description ?? s.filename}</div>
            </div>
            <div className="row-right">
              {s.tags?.slice(0, 2).map((t) => <span key={t} className="tag">{t}</span>)}
